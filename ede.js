...
(async function () {
    ...
    // ------ require start ------
    // 破坏 AMD 机制，防止 Emby 的 requirejs 拦截 Danmaku 挂载
    if (!window.Danmaku) {
    ...
    async function createDanmaku(comments) {
        ...
        const DanmakuEngine = window.Danmaku || (typeof Danmaku !== 'undefined' ? Danmaku : null);
        if (!DanmakuEngine) {
            console.error('Danmaku 引擎未就绪，取消弹幕加载');
            return;
        }

        window.ede.danmaku = new DanmakuEngine({
            container: wrapper,
            media: _media,
            comments: _comments,
